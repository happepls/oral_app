...
import { getLocalDateKey, pickProgressAwareRecallScenario } from '../utils/dailyTaskMaterial';
...
function Recall() {
  ...
  const loadScenario = React.useCallback(async (goalData = null, recallVariant = 0) => {
    ...
    try {
      ...
      try {
        const generated = await aiAPI.getDailyRecall(recallVariant);
