...
function Recall() {
  ...
  const recallAbortRef = useRef(null);

  // Load scenario sentences and translate
  const loadScenario = React.useCallback(async (goalData = null, recallVariant = 0) => {
    ...
    try {
      ...
      const picked = pickProgressAwareRecallScenario(scenarios, {
        goalId: goal?.id,
      ...
      // Primary path: a same-day, progress-aware script generated by the
      // existing AI service. It is cached server-side by user/goal/date/variant,
      // so reloads are stable while explicit switches get fresh material.
      try {
        ...
        console.warn('[Recall] daily material unavailable, using scenario fallback:', e);
      }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const serverSwitchCount = Number(data.switch_count) || 0;
        setSwitchCount(serverSwitchCount);
        setAlreadyCompleted(done);
        if (ignore) return;
        loadScenario(null, serverSwitchCount);
        return;
      } catch (e) {
        ...
        setAlreadyCompleted(done);
        if (ignore) return;
        loadScenario(null, todaySwitches);
        return;
      }
    })();
    return () => { ignore = true; };
  ...
  const handleSwitchScenario = async () => {
    ...
    try {
      ...
      if (Number.isFinite(serverCount)) {
        ...
        localStorage.setItem(`recall_switches_${today}`, String(nextCount));
      }
    } catch (e) {
    ...
    const loaded = await loadScenario(null, nextCount);
    if (loaded) {
      setAlreadyCompleted(false);
    } else {
      setSwitchError('暂时无法生成新句子，请稍后重试。');
    }
    setSwitching(false);
  };
  ...
  const playPreview = async () => {
    ...
    try {
      ...
      audio.onerror = () => {
        release();
        setPreviewError('播放失败，请重试');
      };
      await audio.play();
    } catch (e) {
      console.warn('[Recall] 试听合成失败:', e);
      setPreviewError('语音合成失败，请重试');
    } finally {
      setSynthesizing(false);
    }
  };

  // 切换句子或卸载时停掉试听并释放 Blob URL。
  useEffect(() => () => {
    try { previewAudioRef.current?.pause(); } catch {}
    previewAudioRef.current = null;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen px-6 text-center bg-background-light">
        ...
        {canSwitch ? (
          ...
          >
            {switching ? '正在生成新句子…' : `切换下一组句子练习（剩余 ${FREE_SWITCH_LIMIT - switchCount} 次）`}
          </button>
        ) : (
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            >
              ...
              {previewError && (
                <button type="button" onClick={playPreview} className="text-xs text-red-600 underline">
                  {previewError}
                </button>
              )}
            </StepBlock>
