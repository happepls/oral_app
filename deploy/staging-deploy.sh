...
# Staging Deployment Script for Oral AI Application
...
run_health_checks() {
    ...
    docker-compose -f "$DOCKER_COMPOSE_FILE" exec -T user-service npm test || log "User service tests failed"
    docker-compose -f "$DOCKER_COMPOSE_FILE" exec -T history-analytics-service npm test || log "History analytics service tests failed"
