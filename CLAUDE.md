...
- **⚠️ 部署陷阱（E2E Jun 8 实测发现）**：ai-omni 运行镜像曾**缺 `redis` 库** → `_get_redis_client()` 抛 `ModuleNotFoundError` → 返回 None → 每日上限**与 daily_qa Redis 缓存双双静默 fail-open，从不生效**。根因：`services/ai-omni-service/Dockerfile` 的 `apt-get install gcc g++` 层在阿里云 apt 镜像抖动时 `E: Unable to locate package gcc` 失败 → `--no-cache` rebuild 整体失败，镜像停留在缺 redis/pytest 的旧态。**临时修复**（容器内热装，redis/pytest 纯 python 无需 gcc）：`docker exec -u 0 oral_app_ai_omni_service pip install --no-cache-dir redis==5.0.1 pytest pytest-asyncio -i https://mirrors.aliyun.com/pypi/simple/ --trusted-host mirrors.aliyun.com && docker compose restart ai-omni-service`。**注意**：`docker compose up -d`（recreate）会回退到镜像态 → 再次丢 redis，须重新热装。根治需修 Dockerfile apt 层（让 apt 走宿主 DNS / 容忍镜像抖动 / 或预装 wheel）。
