...
<!-- 工具/Skill/MCP/权限清单不在此维护：harness 每会话自动加载可用 skills、MCP 工具与权限。
     权限 SSOT = `~/.claude/settings.json`（全局）+ `.claude/settings.local.json`（项目）。 -->
...
<!-- 设计集成测试 §9.5 品牌图标 + §10 响应式（Jun 2026）验证记录已全文移入
     `docs/test-cases-design-integration.md`（含测试方法/CDP 仿真坑/截图证据 `docs/mobile-test-screenshots/`）。 -->
