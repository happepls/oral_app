...
const required = ['DATABASE_URL', 'DELEGATED_JWT_SECRET', 'JWT_SECRET'];
...
async function main() {
  await migrate(db);
  const app = createApp({ db, delegatedSecret: process.env.DELEGATED_JWT_SECRET, realtimeSecret: process.env.JWT_SECRET });
