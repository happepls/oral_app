    ...
    <div style={{ height: '100%', overflowY: 'auto', background: DS.bg, paddingBottom: 80 }}>
      ...
      <div style={{ padding: '0 14px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <StatMini emoji="🎯" value="33%" label="总进度" />
        </div>
...
function DailyQAScreen({ onBack }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: DS.bg, position: 'relative' }}>
      ...
      {/* Bottom mic + CC */}
      <div style={{ padding: '12px 16px 18px', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <button onClick={() => setImmersive((v) => !v)} aria-label="toggle immersive" style={{
          width: 56, height: 56, borderRadius: 28, border: `1.5px solid ${immersive ? DS.primary : DS.border}`,
...
function MicBar({ recording, onMic, onRestart, label, secondary }) {
  return (
    <div style={{ padding: '12px 16px 18px', display: 'flex', alignItems: 'center', gap: 10, background: DS.card, borderTop: `1px solid ${DS.border}` }}>
...
function RecallScreen({ onBack, scenario = '询问尺寸与颜色' }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: DS.bg, position: 'relative' }}>
      ...
      {/* Chat view (shown when CC is off) */}
      {!ccMode &&
      <div ref={msgRef} style={{ flex: 1, overflowY: 'auto', padding: '4px 14px 8px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          {messages.map((m, i) =>
        m.type === 'ai' ?
        <AIMessage key={i} text={m.text} duration={m.duration} /> :
        <UserVoiceMessage key={i} duration={m.duration} />
      ...
      {/* CC immersive overlay (default ON) */}
      {ccMode &&
      <div style={{
      ...
      {/* Mic bar — no CC button (CC is default) */}
      <MicBar
        recording={recording}
        onMic={handleMic}
        label="点击跟读"
        onRestart={() => {setMessages(messages.slice(0, 1));setCcMode(true);}} />
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 16px' }}>
        ...
        <>
            ...
            <button key={l.value} onClick={() => setLang(l.value)} style={{
              ...
              borderRadius: DS.radiusMd, padding: '14px 10px',
              display: 'flex', alignItems: 'center', gap: 10,
              cursor: 'pointer', fontFamily: 'Lexend, sans-serif'
            }}>
                  <span style={{ fontSize: 24 }}>{l.flag}</span>
                  <span style={{ fontSize: 14, fontWeight: 600, color: DS.fg }}>{l.label}</span>
                </button>
            )}
            </div>
